import { Brain, CheckCircle, AlertCircle, Eye, Activity } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-2xl p-8 shadow-lg ring-1 ring-slate-200 mt-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Complete Analysis Results</h2>
      </div>
        ...
        <div className="text-center">
          <div
            className={`inline-flex items-center justify-center w-20 h-20 rounded-full mb-4 ${
              results.hasMS ? 'bg-red-100 ring-4 ring-red-200' : 'bg-green-100 ring-4 ring-green-200'
            }`}
          >
            {results.hasMS ? (
          ...
          <div className="inline-block px-6 py-2 bg-blue-100 rounded-full">
            <p className="text-lg font-semibold text-blue-900">
              {(results.probability * 100).toFixed(1)}% Confidence
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="p-4 rounded-lg bg-purple-50 border-2 border-purple-200">
            ...
            <p className="text-3xl font-bold text-purple-700">{(results.mriProb * 100).toFixed(1)}%</p>
            <p className="text-sm text-purple-600 mt-1">Prediction: {results.mriPred}</p>
          </div>
          <div className="p-4 rounded-lg bg-cyan-50 border-2 border-cyan-200">
            ...
            <p className="text-3xl font-bold text-cyan-700">{(results.octProb * 100).toFixed(1)}%</p>
            <p className="text-sm text-cyan-600 mt-1">Prediction: {results.octLabel}</p>
          </div>
          <div className="p-4 rounded-lg bg-blue-50 border-2 border-blue-300">
            <h4 className="font-semibold text-blue-900 mb-2">Combined Result</h4>
            <p className="text-3xl font-bold text-blue-700">{(results.probability * 100).toFixed(1)}%</p>
            <p className="text-sm text-blue-600 mt-1">Prediction: {results.finalPrediction}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8 p-6 bg-slate-50 rounded-xl">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={comparisonData}>
            ...
            <YAxis stroke="#475569" label={{ value: 'Probability (%)', angle: -90, position: 'insideLeft' }} />
            <Tooltip formatter={(value) => `${value.toFixed(1)}%`} />
            <Legend />
            <Bar dataKey="MRI" fill="#a855f7" radius={[8, 8, 0, 0]} />
        ...
        <div className="p-6 bg-slate-50 rounded-xl">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center p-3 bg-white rounded-lg">
              ...
              <span className={`font-bold ${results.mriPred === 'MS' ? 'text-red-600' : 'text-green-600'}`}>
                {results.mriPred}
              </span>
            </div>
            <div className="flex justify-between items-center p-3 bg-white rounded-lg">
              <span className="font-medium text-slate-700">OCT Result:</span>
              <span className={`font-bold ${results.octLabel === 'MS' ? 'text-red-600' : 'text-green-600'}`}>
                {results.octLabel}
              </span>
            </div>
            <div className="flex justify-between items-center p-3 bg-white rounded-lg border-2 border-blue-300">
              <span className="font-medium text-slate-700">Final Diagnosis:</span>
              <span className={`font-bold text-lg ${results.hasMS ? 'text-red-600' : 'text-green-600'}`}>
            ...
            <div className="mt-4 p-3 bg-blue-100 rounded-lg">
              <p className="text-sm text-blue-900 text-center">
                ...
                {results.mriPred === results.octLabel
                  ? 'Both methods agree ✓'
